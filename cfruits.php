...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM `tbl_product` WHERE CONCAT(`id`, `name`, `price`, `type`) LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);
    
}
 else {
    $query = "SELECT * FROM `tbl_product` WHERE type = 'fruit'";
    $search_result = filterTable($query);
}
  		...
  		<div class="col-md-4">
				<form method="post" action="cfruits.php?action=add&id=<?php echo $row["id"]; ?>">
					...
					<div style="border:1px solid #333; background-color:#f1f1f1; border-radius:5px; padding:26px;" align="center">
						...
						<?php echo "Available:"." ". $row['quantity'] ?>
